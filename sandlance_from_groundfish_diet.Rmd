...
As always, we are limited in that there is no data from BC. For now, assign the smallest proportion in the model to the boxes that have zero biomass, for consistency with what has been done for other groups (e.g., capelin).
```{r}
# turn NAs to zeroes for this step
atlantis_box_sl$Sandlance_biomass[is.na(atlantis_box_sl$Sandlance_biomass)] <- 0

sandlance_s1_s4 <- atlantis_box_sl %>% 
  select(.bx0,botz,boundary,Sandlance_biomass) %>% 
  st_set_geometry(NULL) %>%
  mutate(Prop=Sandlance_biomass/sum(Sandlance_biomass,na.rm=TRUE)) %>%
  select(-Sandlance_biomass)
...
Write out a table. 
```{r}
write.csv(sandlance_s1_s4,'sandlance_s1_s4.csv',row.names = FALSE)
```
